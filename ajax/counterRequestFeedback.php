...
//обратная связь для "Счетчика принятий"
...
$value_lim = rrsIblock::getConst('min_counter_req_limit');
...
if(is_array($arGroups)
    ...
    //$ib_id = rrsIblock::getIBlockId('open_counter_feedback');
    $ib_id = 85;
    if(is_numeric($ib_id)){
        ...
        if(intval($new_id) > 0){
            ...
            $arFields = array(
                ...
                'FEEDBACK_URL' => $GLOBALS['host'] . '/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=' . rrsIblock::getIBlockId('open_counter_feedback') . '&type=open_counter&ID=' . $new_id . '&lang=ru&find_section_section=0&WF=Y',
                'TEXT' => ''
            );
            ...
            $arFields['USER_INFO'] = $company_name . $inn_val . '<br/>Тип плательщика: ' . $user_type . '<br/>Количество принятий: ' . $_POST['value'] . '<br/>Ссылка на пользователя в администраторском разделе сайта: ' . $arFields['USER_INFO'].$comment_text;
            ...
            while($data = $res->Fetch()){
                ...
                CEvent::Send('COUNTERREQLIMITSFEEDBACK', 's1', $arFields);
            }
        }
    }
}
