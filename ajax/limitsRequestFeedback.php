...
$value_lim = rrsIblock::getConst('min_request_limit_form');
...
if(is_array($arGroups)
    ...
    $ib_id = rrsIblock::getIBlockId('client_request_limit_feedback');
    if(is_numeric($ib_id)){
        ...
        if(intval($new_id) > 0){
            ...
            $arFields = array(
                ...
                'FEEDBACK_URL' => $GLOBALS['host'] . '/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=' . rrsIblock::getIBlockId('client_request_limit_feedback') . '&type=client&ID=' . $new_id . '&lang=ru&find_section_section=0&WF=Y',
                'TEXT' => ''
            );
            ...
            $arFields['USER_INFO'] = $company_name . $inn_val . '<br/>Тип плательщика: ' . $user_type . '<br/>Количество запросов: ' . $_POST['value'] . '<br/>Стоимость при подаче заявки: ' . ($_POST['value'] * rrsIblock::getConst('request_limit_price')) . ' руб.<br/>Ссылка на пользователя в администраторском разделе сайта: ' . $arFields['USER_INFO'].$comment_text;
            ...
            while($data = $res->Fetch()){
                ...
                CEvent::Send('REQUESTLIMITSFEEDBACK', 's1', $arFields);
            }
        }
    }
}
