...
if($USER->IsAuthorized() && isset($_POST['update_agent_settings']) && $_POST['update_agent_settings'] == 'y')
{
    ...
    if(isset($_POST['client_id'])
        && (filter_var($_POST['client_id'], FILTER_VALIDATE_INT)) !== false
    ...
    )
    {
        ...
        if (in_array(10, $user_groups))
        {
            ...
            if($_POST['agent_id'] > 0){
                ...
                $result = $agentObj->setClientLinkWithPartner($_POST['client_id'], $_POST['agent_id'], $USER->GetID(), (isset($_POST['control_id']) ? $_POST['control_id'] : ''));

            }
            elseif($_POST['agent_id'] == -1){
                //if need to drop farmer link with agent
                $result = $agentObj->dropLinkWithClient($_POST['client_id'], $USER->GetID());
            }
        }
    }
}
