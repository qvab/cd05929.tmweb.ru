...
if(isset($_POST['bl_id'])
    ...
    if($USER->IsAuthorized()){
        ...
        if($data = $res->Fetch()) {
            ...
            $type = deal::getDealType($data['PROPERTY_DEAL_VALUE']); //тип предложения для пары (платное или агентское)
            if($type == 'c'){
                //списываем 1 принятие
                client::counterReqLimitQuantityChange('bl_del', -1, $user_id);
            }
            $result['count'] = client::openerCountGet($user_id);

            //восстанавливаем соответствия
            lead::restoreLeadsFromBlacklist($data['PROPERTY_OPPONENT_VALUE'], $data['PROPERTY_USER_VALUE']);
            ...
            if(count($email_arr) > 0){
                ...
                $user_data = client::getUserData($data['PROPERTY_USER_VALUE']);
                $black_user_data = farmer::getUserData($data['PROPERTY_OPPONENT_VALUE']);

                foreach($email_arr as $email_val => $cur_flag) {
                    $emailFields = array(
                        'TYPE_USER_NAME' => 'Покупатель',
                        'USER_INFO' => $user_data,
