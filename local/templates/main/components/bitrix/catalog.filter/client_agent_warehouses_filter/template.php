...
$agentObj = new agent();
$checkFilter = $agentObj->filterClientAgentWhCheck();
        ...
        if(count($arResult['CLIENTS_LIST']) > 0){
            ...
            <form action="" method="get" name="warehouse_filter" class="select_item">
                <div class="fblock_fm row">
                    <select <?if(count($arResult['CLIENTS_LIST']) > 4){?>data-search="y"<?}?> name="select_client" placeholder="Выберите покупателя">
                ...
                <div class="row fbtn_cancel">
                    <a href="<?=$arParams['LIST_URL'] . (isset($_GET['status']) ? '?status=' . $_GET['status'] : '')?>" class="cancel_filter">Сбросить</a>
                </div>
...
$GLOBALS[$filterName]['PROPERTY_CLIENT'] = (count($selected_clients) > 0 ? $selected_clients : array_keys($arResult['CLIENTS_LIST']));
