...
$isFiltered =  (isset($_GET['warehouse_id'])         && !empty($_GET['warehouse_id']))
                ...
                || (isset($_COOKIE['warehouse_id'])  && !empty($_COOKIE['warehouse_id']))
                || (isset($_COOKIE['culture_id'])    && !empty($_COOKIE['culture_id']))
                || (isset($_COOKIE['region_id'])     && !empty($_COOKIE['region_id']))
                || (isset($_COOKIE['client_id'])     && !empty($_COOKIE['client_id']));
    ...
    <div class="tab_form client_counter_requests">
            ...
            <form method="GET" id="client_requests_filter" data-graphmode="month">
                        ...
                        <div class="wrap-select">
                            <select <?if(count($arResult['CLIENT_LIST']) > 3){?>data-search="y"<?}?> name="client_id" placeholder="Выберите покупателя">
                                ...
                                foreach($arResult['CLIENT_LIST'] as $cur_id => $arClient){
                                    ...
                                    if($arClient['NICK'] != ''){?>
                                        <option data-plimit="<?=$arClient['LIMIT'];?>" <?if(!empty($arClient['CONTRACT'])){?>data-pcontract="1"<?}?> data-cnt="<?=$cur_cnt;?>" value="<?=$cur_id;?>" <?=$sSelected;?> ><?=$arClient['NICK'];?></option>
                                    <?}elseif($arClient['NAME'] == ''){?>
                                        <option data-plimit="<?=$arClient['LIMIT'];?>" <?if(!empty($arClient['CONTRACT'])){?>data-pcontract="1"<?}?> data-cnt="<?=$cur_cnt;?>" value="<?=$cur_id;?>" <?=$sSelected;?> ><?=(!checkEmailFromPhone($arClient['EMAIL']) ? $arClient['EMAIL'] : $cur_id);?></option>
                                    <?}else{?>
                                        <option data-plimit="<?=$arClient['LIMIT'];?>" <?if(!empty($arClient['CONTRACT'])){?>data-pcontract="1"<?}?> data-cnt="<?=$cur_cnt;?>" value="<?=$cur_id;?>" <?=$sSelected;?> ><?=$arClient['NAME'];?><?if(!checkEmailFromPhone($arClient['EMAIL'])){?> (<?=$arClient['EMAIL'];?>)<?}?></option>
                                    <?}
                                }?>
                            </select>
                        </div>
                ...
                <div class="row submit-row">
                    ...
                    <?if($arResult['MODE'] == 'agent'):?>
                        <div class="wrap-btn">
                            <input class="submit-btn reset" value="Сбросить" type="submit">
                        </div>
                    <?endif;?>

                    <div class="show_hide_counter_graph_container <?if($arResult['MODE'] == 'agent'):?>small<?endif;?>">
                        <div class="show_hide_counter_graph" onclick="showClientGraph(0);" date-textshow="График цен по культуре<br> на текущем складе" date-texthide="Скрыть график">График цен по культуре  <?if($arResult['MODE'] == 'agent'):?><br><?endif;?> на текущем складе</div>
                    </div>
                    <div class="clear"></div>
                </div>
        ...
        <div id="draw_object_block_old"><?//удалить _old чтобы работал фикс-скролл?>
            <div id="draw_object">
                ...
                <div class="graph_area" data-viewmode="week"></div>
            </div>
        </div>
        <?endif;?>
