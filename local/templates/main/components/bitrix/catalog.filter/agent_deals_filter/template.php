...
<div class="tab_form">
    ...
    if(count($arResult['FARMERS_LIST']) > 0){
        ...
        ?>
        <form action="" method="get" class="select_item" name="offers_filter">
            ...
            <div class="row fbtn_cancel">
                <a href="/agent/deals/<?=(isset($_GET['status']) ? '?status=' . $_GET['status'] : '')?>" class="cancel_filter">Сбросить</a>
            </div>
...
$GLOBALS[$filterName]['PROPERTY_'.$arParams['USER_TYPE']] = (isset($_GET['farmer_id']) ? $_GET['farmer_id'] : array_keys($arResult['FARMERS_LIST']));
