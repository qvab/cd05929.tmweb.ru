...
if (sizeof($arResult["ITEMS"]) > 0) {
    ...
    <div class="additional_href_data">
        <a href="/partner/farmer_warehouses/add/">+ Добавить склад</a>
    </div>
                ...
                <div class="line_inner">
                    <div class="name"><?=$arItem['NAME']?>
                        <?if(isset($arResult['FARMERS_DATA'][$arItem['PROPERTIES']['FARMER']['VALUE']])){
                            ...
                            if($cur_farmer['NICK'] != ''){?>
                                <div class="farmer_name"><?=$cur_farmer['NICK'];?></div>
                            <?}elseif($cur_farmer['NAME'] == ''){?>
                                <div class="farmer_name"><?=(!checkEmailFromPhone($cur_farmer['EMAIL']) ? $cur_farmer['EMAIL'] : $arItem['PROPERTIES']['FARMER']['VALUE']);?></div>
                            <?}else{?>
                                <div class="farmer_name"><?=$cur_farmer['NAME'];?><?if(!checkEmailFromPhone($cur_farmer['EMAIL'])){?> (<?=$cur_farmer['EMAIL'];?>)<?}?></div>
                            <?}
                        }
                        ?>
                    </div>
...
elseif(count($arResult['FARMERS_DATA']) == 0) {
?>
...
if(count($arResult['FARMERS_DATA']) > 0){
    ?>
    <a href="/partner/farmer_warehouses/add/" class="add_blue_button warehouses">Добавить склад<div class="but_addit"><div class="ico">+</div></div></a>
    <?
}?>
