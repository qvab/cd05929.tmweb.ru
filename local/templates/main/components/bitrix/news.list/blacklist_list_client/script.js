...
function deactivateItem(item_id) {
    ...
    $.ajax({
        type: "POST",
        url: "/ajax/client_blacklist_del.php",
        data: {
            bl_id: item_id
        },
        dataType: 'JSON',
        success: function(msg){
            ...
            if(msg.result == 1){
                ...
                if(msg.count>0){
                    $('.opening_limit_available .limit_val').text(msg.count);
                }else{
                    $('.opening_limit_available').hide();
                    $('.opening_limit_ended').show();
                }
            }else{
...
$(document).ready(function() {
    ...
    $('.del_bl').on('click',function () {
        if($(this).attr('data-id').length>0){
            ...
            var agent_offer_flag = ($(this).hasClass('agent_offer'));
            ...
            $('#page_body').append('<div id="popup_phone_num" class="no_round_pop y_n_popup">' +
                    ...
                    '<div class="popup_header">Удаление поставщика из черного списка</div>' +
                    (agent_offer_flag
                        ? '<div class="text">Ваши запросы вновь будут видны поставщику.</div>'
                        : '<div class="text">При удалении поставщика из черного списка у вас будет списано одно принятие, а также ваши запросы вновь будут видны поставщику.</div>'
                    ) +
