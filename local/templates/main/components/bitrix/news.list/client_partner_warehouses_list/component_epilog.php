...
if ($_REQUEST['warehouse'] > 0) {
    ...
    $client_id = partner::getClientByWarehouse($_REQUEST['warehouse']);

    if($client_id > 0){
        global $USER;
        //проверка прав агента на действие от лица покупателя
        $user_data = partner::getClientsForSelect($USER->GetID(), $client_id, false, false, false);
    ...
    if($has_right){
        ...
        if($IB_WH_ID){
            ...
            if ($res->SelectedRowsCount() > 0) {
                //если склад неактивный то деактивируем запросы (также удаляем пары и встречные предложения)
