...
if (sizeof($arResult["ITEMS"]) > 0) {
    ...
    if(count($arResult["ITEMS"]) > 10 && $arParams['HAS_ADD_RIGHTS']):?>
        <div class="additional_href_data">
            <a href="/client_agent/warehouses/add/">+ Добавить склад</a>
        </div>
    <?endif;?>
            ...
            <div class="line_area <?=$arItem["PROPERTIES"]['ACTIVE']['VALUE_XML_ID']?>">
                    ...
                    <div class="name"><?=$arItem['NAME']?>
                        <?if(isset($arResult['CLIENTS_DATA'][$arItem['PROPERTIES']['CLIENT']['VALUE']])){
                            ...
                            <?}elseif($cur_client['NAME'] == ''){?>
                                <div class="client_name"><?=$cur_client['EMAIL'];?></div>
                            <?}else{?>
                                <div class="client_name"><?=$cur_client['NAME'];?> (<?=$cur_client['EMAIL'];?>)</div>
                            <?}
                            }
                        ?>
                    </div>
                ...
                <form action="" method="post" class="line_additional" data-lat="<?=$map[0]?>" data-lng="<?=$map[1]?>">
                    ...
                    <div class="prop_area additional_submits">
                        <?
                        if(isset($arResult['CLIENTS_DATA'][$arItem['PROPERTIES']['CLIENT']['VALUE']]['LINK_DOC'])
                            && $arResult['CLIENTS_DATA'][$arItem['PROPERTIES']['CLIENT']['VALUE']]['LINK_DOC'] == 'Y'
                        ){
...
if(isset($arParams['HAS_ADD_RIGHTS'])
    && $arParams['HAS_ADD_RIGHTS']
){?>
    <a href="/client_agent/warehouses/add/" class="add_blue_button warehouses">Добавить склад<div class="but_addit"><div class="ico">+</div></div></a>
<?}?>
