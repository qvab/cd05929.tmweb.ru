...
if (sizeof($arResult["ITEMS"]) > 0) {
    ...
    if(count($arResult["ITEMS"]) > 10):?>
        <div class="additional_href_data">
            <a href="/agent/warehouses/add/">+ Добавить склад</a>
        </div>
    <?endif;?>
                    ...
                    <div class="name"><?=$arItem['NAME']?>
                        <?if(isset($arResult['FARMERS_DATA'][$arItem['PROPERTIES']['FARMER']['VALUE']])){
                            ...
                            <?}elseif($cur_farmer['NAME'] == ''){?>
                                <div class="farmer_name"><?=$cur_farmer['EMAIL'];?></div>
                            <?}else{?>
                                <div class="farmer_name"><?=$cur_farmer['NAME'];?> (<?=$cur_farmer['EMAIL'];?>)</div>
                            <?}
                        }
                        ?>
                    </div>
...
if(count($arResult['FARMERS_DATA']) > 0){
    ?>
    <a href="/agent/warehouses/add/" class="add_blue_button warehouses">Добавить склад<div class="but_addit"><div class="ico">+</div></div></a>
    <?
}?>
