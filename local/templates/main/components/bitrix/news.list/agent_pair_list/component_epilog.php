    ...
    if(count($arUpdatePropertyValues) > 0){
        ...
        $res = CIBlockElement::GetList(
            ...
            array('ID', 'NAME', 'PROPERTY_IS_ADD_CERT', 'PROPERTY_IS_BILL_OF_HEALTH', 'PROPERTY_IS_VET_CERT', 'PROPERTY_IS_QUALITY_CERT',
                'PROPERTY_IS_TRANSFER', 'PROPERTY_IS_SECURE_DEAL', 'PROPERTY_IS_AGENT_SUPPORT', 'PROPERTY_FARMER', 'PROPERTY_CLIENT', 'PROPERTY_OFFER')
        );
        if($ob = $res->Fetch()) {
            $offerId = $ob['PROPERTY_OFFER_VALUE'];
            $farmerId = $ob['PROPERTY_FARMER_VALUE'];
            $clientId = $ob['PROPERTY_CLIENT_VALUE'];
        ...
        $agentObj = new agent();
        //партнеры покупателя
        $clientPartners = $agentObj->getProfileListByClientID($clientId);
        if((sizeof($clientPartners))&&(is_array($clientPartners))){
            foreach ($clientPartners as $partner_id=>$partner){
                if(
                    ...
                    $arEventFields = array(
                        'FIO'   => $sUserName,
                        'URL'   => $GLOBALS['host'].'/profile/?uid=' . $clientId,
                        'LIST'  => $sList,
...
if(isset($arResult['ELEMENTS'])
    ...
    if($arParams['USER_TYPE'] == 'CLIENT'){
        $opp_arr = client::getUserBlackList($USER->GetID());
    }else{
        $opp_arr = farmer::getUserBlackList($USER->GetID());
    }
