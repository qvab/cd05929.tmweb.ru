...
$(document).ready(function() {
    ...
    $('.list_page_rows').on('keyup', '.line_additional input[name="volume"]', function(){
    ...
    $('.list_page_rows').on('keyup change', '.line_additional input[name="price"]', function(){
        var wObj = $(this);
        setTimeout(function(){
    ...
    $('.list_page_rows').on('change', 'input[name="partner_service_price"]', function(){
    ...
    $('.list_page_rows').on('click', '.counter_request_additional_data input[type="button"][name="save"]', function(){
        ...
        if(coffer_type === 'p') {
            ...
            wObj = wObjArea.find('input[name="IS_ADD_CERT"]');
            if (wObj.length == 1
                && wObj.prop('checked') === true
            ) {
                addit_IS_ADD_CERT = 1;
            }
            wObj = wObjArea.find('input[name="IS_BILL_OF_HEALTH"]');
            if (wObj.length == 1
                && wObj.prop('checked') === true
            ) {
                addit_IS_BILL_OF_HEALTH = 1;
            }
            wObj = wObjArea.find('input[name="IS_VET_CERT"]');
            if (wObj.length == 1
                && wObj.prop('checked') === true
            ) {
                addit_IS_VET_CERT = 1;
            }
            wObj = wObjArea.find('input[name="IS_QUALITY_CERT"]');
            if (wObj.length == 1
                && wObj.prop('checked') === true
            ) {
                addit_IS_QUALITY_CERT = 1;
            }
            wObj = wObjArea.find('input[name="IS_TRANSFER"]');
            if (wObj.length == 1
                && wObj.prop('checked') === true
            ) {
                addit_IS_TRANSFER = 1;
            }
            wObj = wObjArea.find('input[name="IS_SECURE_DEAL"]');
            if (wObj.length == 1
                && wObj.prop('checked') === true
            ) {
                addit_IS_SECURE_DEAL = 1;
            }
...
function stopCounterOffer(iOfferId){
    $.post('/ajax/cancelCounterOffer.php', {
        ...
        var objInput = $('.list_page_rows input[type="hidden"][name="offer"][value="' + iOfferId + '"]');
