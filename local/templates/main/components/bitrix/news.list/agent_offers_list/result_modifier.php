...
$arResult['STATUS_AVAILABLE_LIST'] = rrsIblock::getPropListKey('farmer_offer', 'STATUS_AVAILABLE');
$agentObj = new agent();
...
{
    $user_ids[$cur_item['PROPERTIES']['FARMER']['VALUE']] = true;
}

if(count($user_ids) > 0)
{
    $arResult['FARMERS_DATA'] = $agentObj->getFarmersForSelect($arParams['UID']);
    $arResult['FARMERS_PROFILE_DONE'] = $agentObj->getFarmersRegistrationRights(array_keys($arResult['FARMERS_DATA']));
