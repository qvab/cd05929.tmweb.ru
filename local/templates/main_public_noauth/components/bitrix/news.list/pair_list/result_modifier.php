...
<?
$userIds = array();     //ID клиентов/поставщиков в зависимости от типа пользователя
$partnersIds = array(); //ID партнеров
$thisIds = array();      //ID текущего пользователя (делаем через массив для совместимости)
...
if ($arParams['USER_TYPE'] == 'FARMER') {
    ...
    foreach ($arResult['ITEMS'] as $i => $arItem) {
        ...
        if ($arItem['PROPERTIES']['FARMER']['VALUE'] > 0) {
            $thisIds[$arItem['PROPERTIES']['FARMER']['VALUE']] = true;
        }
        ...
        if($arResult['ITEMS'][$i]['PAIR_TYPE'] == 'p'){
            if(!empty($arItem['PROPERTIES']['COFFER_BY_PARTNER']['VALUE'])){
                ...
                $arResult['ITEMS'][$i]['PARTNER_ID'] = $arItem['PROPERTIES']['COFFER_BY_PARTNER']['VALUE'];
            }
        }
    }
}
...
//        if($arResult['ITEMS'][$i]['PAIR_TYPE'] == 'p'){
        if (!empty($arItem['PROPERTIES']['DEAL_REFERER']['VALUE'])) {
            ...
            $arResult['ITEMS'][$i]['PARTNER_ID'] = $arItem['PROPERTIES']['DEAL_REFERER']['VALUE'];
        } else {
            if ($arItem['PROPERTIES']['CLIENT']['VALUE'] > 0) {
                //получаем последнего орг. по дате привязки
                $partners = client::getLinkedPartnerList($arItem['PROPERTIES']['CLIENT']['VALUE'], true);
                if (sizeof($partners) > 0) {
                    $partnersIds[$partners[0]] = true;
                    $arResult['ITEMS'][$i]['PARTNER_ID'] = $partners[0];
                }
            }
        }
        if ($arItem['PROPERTIES']['CLIENT']['VALUE'] > 0) {
            $thisIds[$arItem['PROPERTIES']['CLIENT']['VALUE']] = true;
        }
//        }
        if ($arItem['PROPERTIES']['FARMER']['VALUE'] > 0) {
...
if (is_array($userIds) && sizeof($userIds) > 0) {
    $user_type_ip = rrsIblock::getPropListKey($ibCode, 'UL_TYPE', 'ip');
    ...
    while($ob = $res->Fetch()){
        if(isset($arResult['USER_LIST'][$ob['ID']])){
            $arResult['USER_LIST'][$ob['ID']]['USER_DATA']['F_NAME'] = trim($ob['LAST_NAME'].' '.$ob['NAME'].' '.$ob['SECOND_NAME']);
            $arResult['USER_LIST'][$ob['ID']]['USER_DATA']['NAME'] = trim($ob['NAME']);
        }
    }
}
if (is_array($thisIds) && sizeof($thisIds) > 0) {
    $user_type_ip = rrsIblock::getPropListKey($thisIbCode, 'UL_TYPE', 'ip');

    $res = CIBlockElement::GetList(
        array('ID' => 'ASC'),
        array(
            'IBLOCK_ID' => rrsIblock::getIBlockId($thisIbCode),
            'ACTIVE' => 'Y',
            'PROPERTY_USER' => array_keys($thisIds)
        ),
        false,
        false,
        array(
