...
/*
 * Создает ИБ для хранения привязки "Региональный менеджер"-"Организатор"
 *
...
$arIBlockTypes = [
    // Имя типа ИБ
    [
        'CODE'      => 'REGIONAL_MANAGERS',
        'DELETE'    => false,    // Удалить этот ИБ? (удален будет перед тем как создан, таким образом можно пересоздать ИБ)
        'PROPERTY'  => [
            'SECTIONS' => 'N',
            'IN_RSS'   => 'N',
            'SORT'     => 1160,
...
$arIBlocks = [
    ...
    [
        'TYPE'      => 'REGIONAL_MANAGERS',
        'CODE'      => 'BIND_REGIONAL_TO_ORGANIZERS',
        'CREATE'    => [   // Если это свойство задано, то если такого ИБ нет то он будет создан
            ...
            'BIZPROC' => 'N',
            'ACTIVE'  => 'Y',
            'NAME'    => 'Привязка к организаторам',
            'GROUP_ID'=> ['1' => 'X','2' => 'R',],
            'FIELDS' => [
                'NAME'        => [
                    'IS_REQUIRED'   => 'N',
                    'DEFAULT_VALUE' => 'Привязка регионального менеджера к организатору',
                ],
            ],
        ...
        'TABS'       => [
            ...
            [
                ...
                'TITLE'  => 'Привязка',             // Если не задан или задан в true и указан CODE стандартного таба, то будет взято стандартное имя
                // Поля в табе
                'FIELDS' => [                       // Если вместо массива указать true или свойство вообще отсутсвует и указан CODE стандартного таба, то будет подставлен стандартный набор для этого раздлела, особенно актуально для SEO
                    ...
                    'edit1_csection1'           => '--Связь',    // Для заголовков блоков вначале нужно писать две черточки (--), а код должен быть уникальным для всех табов
                    'PROPERTY_REGIONAL_MANAGER' => true,            // Для свойств можно указать либо PROPERTY_*ID*, либо PROPERTY_*CODE*
                    'PROPERTY_ORGANIZER'        => true,            // Если указывается код, то допускается для автоподстановки имени указывать true
                ],
            ],
        ],
    ],
