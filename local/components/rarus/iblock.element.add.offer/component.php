...
if ($bAllowAccess) {
    ...
    if ($arParams["ID"] > 0) {
        ...
        $rsIBlockElements = CIBlockElement::GetList(array("SORT" => "DESC"), $arFilter);
...
if ($bAllowAccess) {
    ...
    if (!empty($_REQUEST["save"])) {
        ...
        /*// Проверяем все ли документы есть у АП
        $obDoc = new CDocument;
        $bIsFullSetDocument = $obDoc->isFullSet($farmerId, 'farmer');*/
        ...
        if($bIsFullSetDocument) {
            ...
            $arUpdateValues["MODIFIED_BY"] = $farmerId;
            ...
            $arDocument = $obDoc->getDocumentsFarmer($farmerId);
            $sErrorDoc = 'Для создания товара добавьте следующие документы в систему:' . PHP_EOL;
        ...
        if (empty($arResult["ERRORS"])) {
            ...
            //поиск подходящих для товара запросов покупателей
    ...
    $arResult['WAREHOUSE_LIST'] = farmer::getWarehouseList($USER->GetID());
