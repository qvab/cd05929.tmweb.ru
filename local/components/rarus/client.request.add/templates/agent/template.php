...
<form name="iblock_add" class="request_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" data-nds="<?=$arResult['NDS_VAL'];?>">
        ...
        <div class="row_val">
            <select <? if (count($arResult['CLIENTS_DATA']) > 3) { ?>data-search="y"<? } ?> name="client_id" placeholder="Выберите поставщика">
                ...
                foreach($arResult['CLIENTS_DATA'] as $cur_id => $cur_data){
                    ...
                    if($cur_data['NICK'] != ''){?>
                        <option data-right="<?=$user_right?>" data-limit="<?=$limit_val;?>" value="<?=$cur_id;?>" <?if($cur_active){?>selected="selected"<?}?> ><?=$cur_data['NICK'];?></option>
                    <?}elseif($cur_data['NAME'] == ''){?>
                        <option data-right="<?=$user_right?>" data-limit="<?=$limit_val;?>" value="<?=$cur_id;?>" <?if($cur_active){?>selected="selected"<?}?> ><?=(!checkEmailFromPhone($cur_data['EMAIL']) ? $cur_data['EMAIL'] : $cur_id);?></option>
                    <?}else{?>
                        <option data-right="<?=$user_right?>" data-limit="<?=$limit_val;?>" value="<?=$cur_id;?>" <?if($cur_active){?>selected="selected"<?}?> ><?=$cur_data['NAME'];?><?if(!checkEmailFromPhone($cur_data['EMAIL'])){?> (<?=$cur_data['EMAIL'];?>)<?}?></option>
                    <?}
                }?>
            </select>
        </div>
    ...
    <div class="spec_row_default">
        <div class="row">
            <div <?=(isset($arResult['ACTIVE_REQ']) ? 'data-actid="' . $arResult['ACTIVE_REQ'] . '"' : '');?> class="form_line_error limit">У выбранного Вами покупателя исчерпан лимит создания запросов</div>
        ...
        <div class="row">
            <input type="button" name="iblock_submit" class="submit-btn <? if ($arResult['SAVE'] == 'Y' && $qw > 0) { ?> active<? } else { ?> inactive<? } ?>" value="<? if($arParams['ID']>0) echo GetMessage("IBLOCK_FORM_SUBMIT_EDIT"); else echo  GetMessage("IBLOCK_FORM_SUBMIT");  ?>" />
        </div>
    </div>
    <?/*
    if ($GLOBALS['DEMO'] != 'Y') {
