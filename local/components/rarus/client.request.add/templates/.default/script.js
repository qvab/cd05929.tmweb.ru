...
$(document).ready(function() {
    ...
    $('select[name="payment"], select[name="delivery"]').on('change', function(){
...
function rrsCheckSubmit()
{
    ...
    var checked_rules_flag = ($('#agree-cost').prop('checked') === true && $('#agreement').prop('checked') === true);
    var wObj = $('.submit-btn');

    if(checked > 0)
    {
        if(checked_rules_flag && !wObj.hasClass('total_disabled'))
        {
        ...
        if(!wObj.hasClass('total_disabled')) {
...
function checkParamsCorrect(wObj, mode)
{
        ...
        {//dump parameter was changed
            ...
            {//interface input or keyboard input need check
                ...
                {//dump min parameter was changed
                    ...
                    //check if there another minimum value, that have position lower that current
                    var same_value_at_another_position = false;
                    var check_cur_val = parseFloat(val);
                    var cur_index_pos = wObj.parents('.quality-dump-table-item').index();
                    wObj.parents('.quality-dump-table-item').siblings('.quality-dump-table-item').each(function(temp_ind, temp_c_obj){
                        if(cur_index_pos > $(temp_c_obj).index()
                            && (check_cur_val < parseFloat($(temp_c_obj).find('input[type="text"][name*="MIN"]').val())
                                ||check_cur_val < parseFloat($(temp_c_obj).find('input[type="text"][name*="MAX"]').val())
                            )
                        ){
                            //found another min that have lower index and higher value
                            wObj.val($(temp_c_obj).find('input[type="text"][name*="MAX"]').val());
                            same_value_at_another_position = true;
                            return false;
                        }
                    });
                    if(same_value_at_another_position){
                        //found another min that have lower index and higher value
                        return false;
                    }
                ...
                {//dump max parameter was changed
                    ...
                    //check if there another maximum value, that have position higher that current
                    var same_value_at_another_position = false;
                    var check_cur_val = parseFloat(val);
                    var cur_index_pos = wObj.parents('.quality-dump-table-item').index();
                    wObj.parents('.quality-dump-table-item').siblings('.quality-dump-table-item').each(function(temp_ind, temp_c_obj){
                        if(cur_index_pos < $(temp_c_obj).index()
                            ...
                            //found another max that have higher index and lower value
                            wObj.val($(temp_c_obj).find('input[type="text"][name*="MIN"]').val());
                            same_value_at_another_position = true;
                            return false;
                        }
                    });
    ...
    if(typeof temp_val[1] != 'undefined' && temp_val[1].toString().length > 1)
    {
        ...
        wObj.val(parseFloat(val.replace(' ', '')));
    }
    ...
    if(
        ...
        if(
            ...
            if(bSnoNds){
                ...
                nNewPrice = iCurrentPrice + (iCurrentPrice * 0.01 * nNdsVal);
            }else{
