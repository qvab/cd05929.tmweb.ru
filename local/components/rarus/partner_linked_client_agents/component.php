...
if ($arResult['ERROR'] != 'Y') {
    ...
    $arResult['ITEMS'] = $agentObj->getClientAgentsOfPartner($u_id);
        ...
        if ($arResult['ITEMS'][$_GET['resend']]['ACTIVE'] == 'Y') {
            ...
            if ($data = $res->Fetch()) {
                if ($data['ACTIVE'] == 'Y') {
                    ...
                    $arEventFields = array(
                        'EMAIL' => $data['EMAIL'],
                        'HREF'  => $GLOBALS['host'] . '/?reg_hash=' . $data['UF_HASH_INVITE'] . rrsIblock::getIBlockId('client_agent_profile') . '#action=register',
                        'TO'    => 'агента'
                    );
    ...
    {
        if($agentObj->deleteClientAgent($_GET['unlink_agent'], $u_id))
        {
    ...
    if (isset($_GET['unlink_partner_del']) && is_numeric($_GET['unlink_partner_del']) && isset($arResult['ITEMS'][$_GET['unlink_partner_del']])) {//delete user
        $agentObj->deleteClientAgent($_GET['unlink_partner_del'], $u_id);
    ...
    )
    {
        ...
        $res = $el_obj->GetList(array('ID' => 'ASC'), array('IBLOCK_ID' => rrsIblock::getIBlockId('client_agent_partner_link'), 'PROPERTY_PARTNER_ID' => $u_id, 'PROPERTY_USER_ID' => $_POST['uid']), false, array('nTopCount' => 1), array('ID', 'IBLOCK_ID'));
        if($data = $res->Fetch())
        {
