    ...
    if(!empty($_GET['SAVE_OPTIONS']) && $_GET['SAVE_OPTIONS'] == 'Y') {
        ...
        try {
            ...
            if(empty($arProfile['ID'])) {
                throw new Exception('Не удалось получить профиль агента пользователя['.$iAgentId.']');
            }
            ...
            $nPercentTransportation = trim($_GET['PERCENT_TRANSPORTATION']);
            if(strlen($nPercentTransportation) > 0) {
                $nPercentTransportation = floatval($_GET['PERCENT_TRANSPORTATION']);
            }

            // Обновляем значение св-ва
            $obEl->SetPropertyValuesEx(
                $arProfile['ID'],
                getIBlockID('agent', 'agent_profile'),
                [
                    'REWARD_PERCENT'            => $nPercent,
                    'PERCENT_TRANSPORTATION'    => $nPercentTransportation,
                ]
            );
            ...
            $arData = $obEl->GetList(
                ...
                false,
                false,
                ['ID', 'PROPERTY_REWARD_PERCENT', 'PROPERTY_PERCENT_TRANSPORTATION',]
            )->Fetch();
            ...
            if($arData['PROPERTY_PERCENT_TRANSPORTATION_VALUE'] != $nPercentTransportation) {
                throw new Exception('Ошибка записи значения поля "Процент вознаграждения за транспортировку" текущее значение = "'.$arData['PROPERTY_PERCENT_TRANSPORTATION_VALUE'].'", ожидаемое "'.$nPercentTransportation.'"');
            }
