...
if(isset($_POST['PUBLIC_FORM'])
    ...
    if(empty($arResult['ERROR_MESSAGE'])){
        ...
        $arFields = array(
            'IBLOCK_ID' => rrsIblock::getIBlockId('agrohelper_team'),
            'ACTIVE' => 'Y',
            'NAME' => 'Новая заявка на участие в агрохелпер от ' . $_POST['email'],
        ...
        if($new_id = $el_obj->Add($arFields)){
            ...
            if(count($email_arr) > 0) {
                CEvent::Send('BECOME_AGROHELPER', 's1', array(
            ...
            if(count($admin_email_arr) > 0) {
                CEvent::Send('PARTNER_BECOME_AGROHELPER', 's1', array(
            ...
            setcookie('become_agrohelper_success', 'y', time() + 60);
            global $APPLICATION;
            LocalRedirect($APPLICATION->GetCurUri(false, false) /*. '#action=become_agrohelper'*/);
            exit;
        }
    }
}
...
if ($this->StartResultCache()) {
    ...
    if(isset($_COOKIE['become_agrohelper_success'])
        && $_COOKIE['become_agrohelper_success'] == 'y'
    ){
        unset($_COOKIE['become_agrohelper_success']);
        setcookie('become_agrohelper_success', '', -1);
        $arResult['success'] = 'ok';
    }

    if(!empty($arResult['ERROR_MESSAGE'])
        || !empty($arResult['success'])
    ){
