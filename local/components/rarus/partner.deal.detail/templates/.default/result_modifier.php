...
$weight = $fullCost = $fullCsm = $fullTransportCost = 0;
$res = CIBlockElement::GetList(
    array('ID' => 'ASC'),
    array(
...
while ($ob = $res->GetNext()) {
    $arResult['VI'][] = array(
        ...
        'CSM' => $ob['PROPERTY_CSM_VALUE'],
    );

    $weight += $ob['PROPERTY_WEIGHT_VALUE'];
    $fullCost += $ob['PROPERTY_COST_VALUE'];
    $fullCsm += $ob['PROPERTY_CSM_VALUE'];
    $fullTransportCost += $ob['PROPERTY_TRANSPORT_COST_VALUE'];
}

$rc_ = $fullCost / $weight;

$transportTarif = $arResult['PROPERTIES']['TARIF']['VALUE'];
$csm_ = 1000 * $rc_ - $transportTarif;
