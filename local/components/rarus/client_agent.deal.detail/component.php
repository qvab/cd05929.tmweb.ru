...
$arParams['SELF_URL'] = '/client_agent/deals/' . $arParams['ELEMENT_ID'] . '/';
...
if ($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()),$bUSER_HAVE_ACCESS, $arNavigation, $pagerParameters))) {
	...
	if ($obElement = $rsElement->GetNextElement()) {
        ...
        $check_rights = false;
        $client_data = $agentObj->getClientsForSelect($USER->GetID(),$agentObj->$arResult["PROPERTIES"]["CLIENT"]["VALUE"], true, true, true);
        if($agentObj->getClientDealsRightsForAgent($USER->GetID(), $client_data) == 'Y'){
            $check_rights = true;
        }
        ...
        $agentObj = new agent();
        $arResult['LOGS'] = log::getDealStatusLog($arParams['ELEMENT_ID']);
