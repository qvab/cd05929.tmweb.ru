...
if ($arResult['ERROR'] != 'Y') {
    ...
    if (isset($_POST['update']) && $_POST['update'] == 'y') {
        ...
        if ($arResult['ERROR_TEXT'] == '') {
            ...
            if ($arResult['ERROR'] != 'Y' && count($arUpdateProps) > 0) {
                //get profile id
                $res = $el_obj->GetList(
                    array('ID' => 'ASC'),
                    array(
        ...
        if ($arResult['ERROR'] != 'Y') {
            $url = $GLOBALS['APPLICATION']->GetCurDir(false)."?success=ok";
            if(isset($arParams['BY_AGENT']) && $arParams['BY_AGENT'] == 'Y'){
                $url .= '&uid=' . $arParams['U_ID'];
            }
            LocalRedirect($url);
            exit;
        }
    ...
    if ($arParams['TYPE'] == 2) {
        ...
        if ($ulType == 'ul') {
            $arFilter['PROPERTY_NDS'] = $arResult['SHOW_PROPS']['NDS'];
        }
