...
$(document).ready(function(){
    ...
    $('form[name="profile_form"]').on('submit', function(e){
            ...
            if (checkObj.length == 1 && checkObj.parents('.sign_block.no_active').length == 0) {
                ...
                if (err_val != '') {
                    ...
                    if (checkObj.parents('.row').find('.row_err').length > 0) {
                        checkObj.parents('.row').addClass('error').find('.row_err').html(err_val);
                    }
                    else {
        ...
        if(!found_err && checkObj.length == 1 && checkObj.prop('checked') === false)
        {
        ...
        if (name == 'PROP__INN') {
            ...
            if (len != 10 && len != 12) {
                ...
                if(typeof nds == 'undefined'){
                    var nds_addit = $('input[data-name="NDS_VAL"]');
                    if(nds_addit.length == 1
                        && nds_addit.attr('data-val') != 'undefined'
                        && nds_addit.attr('data-val') != ''
                        && nds_addit.attr('data-val').toString().replace(/[0-9]+/g, '') == ''
                    ){
                        nds = nds_addit.attr('data-val');
                    }
                }
                ...
                var old_docs = form.find('.docs_line');
                if(old_docs.length > 0){ form.find('.docs_line').remove(); }
                if (type != '') {
                    $.ajax({
                        type: "POST",
    ...
    //проверка необходимости вывода документов в случае если форма была ранее отправлена с ошибками (например дубль email)
    if($('#page_body .err_text').length > 0){

        var no_send_data_flag = false;
        var form = $('form[name="profile_form"]');
        var user_type_obj = form.find('input[name="TYPE"]');
        var user_type = '';
        var nds_obj = form.find('input[data-name="NDS_VAL"]');
        var nds = '';
        ...
        if(user_type_obj.length > 0 && user_type_obj.val() != ''){ user_type = user_type_obj.val(); }
        else { no_send_data_flag = true; }

        if(!no_send_data_flag && nds_obj.length > 0
            && typeof nds_obj.attr('data-val') != 'undefined'
            && nds_obj.attr('data-val') != ''
        ){ nds = nds_obj.attr('data-val'); }
        else { no_send_data_flag = true; }

        if(!no_send_data_flag && inn_obj.length > 0 && inn_obj.val() != ''){ inn_val = inn_obj.val(); }
        else { no_send_data_flag = true; }

        if(!no_send_data_flag){
            if (inn_val.length == 10) {
                type = 'ul';
            } else if (inn_val.length == 12) {
                type = 'ip';
            } else {
                no_send_data_flag = true;
            }
        }
