...
if ($arResult['ERROR'] != 'Y') {
    ...
    $arResult['CHANGE_NDS'] = farmer::isChangeNDS($arParams['U_ID']);
    ...
    if (isset($_POST['update']) && $_POST['update'] == 'y') {
        ...
        $demo_mode = false;
        $res = $u_obj->GetList(($by="id"), ($order="asc"), array('ID' => $arParams['U_ID']), array('FIELDS' => array(), 'SELECT' => array('UF_DEMO')));
        if ($data = $res->Fetch()) {
            if($data['UF_DEMO']){
                $demo_mode = true;
            }
        ...
        if ($arResult['ERROR_TEXT'] == '') {
            ...
            $arUpdateProps['UL_TYPE'] = rrsIblock::getPropListKey('farmer_profile', 'UL_TYPE', $_POST['PROP__UL_TYPE']);
            if(!filter_var($arUpdateProps['UL_TYPE'], FILTER_VALIDATE_INT)){ unset($arUpdateProps['UL_TYPE']); }
            ...
            if (count($arUpdateProps) > 0) {
                ...
                $res = $el_obj->GetList(
                    ...
                    false,
                    array('nTopCount' => 1),
                    array('ID', 'IBLOCK_ID', 'PROPERTY_PARTNER_ID', 'PROPERTY_VERIFIED')
                );
                if ($data = $res->Fetch()) {
                    if (intval($data['PROPERTY_PARTNER_ID_VALUE']) > 0
                        && $data['PROPERTY_VERIFIED_ENUM_ID'] != rrsIblock::getPropListKey('farmer_profile', 'VERIFIED', 'yes')
                    ) {
                        $arUpdateProps['PARTNER_ID_TIMESTAMP'] = time();
                    }
                    ...
                    && sizeof($_FILES) > 0)
                    {
                        foreach ($_FILES as $key => $file) {
                            if ($file['error'] < 1
                                && isset($file['tmp_name'])
                                && mb_strlen($key) != mb_strlen(str_replace('PROP__', '', $key))
                            ){
                                $arUpdateProps[str_replace('PROP__', '', $key)] = array(
                                    'VALUE'         => $file,
...
//                                );
//                                CEvent::Send('PARTNER_FARMER_FULL_MODE', 's1', $arEventFields);
//                            }
//                            if (in_array($noticeList['c_l']['ID'], $partnerProfile['PROPERTY_NOTICE_VALUE'])) {
//                                notice::addNotice($partnerProfile['USER']['ID'], 'l', 'Переход в полноценный режим работы', $url, '#' . $arParams['U_ID']);
//                            }
...
//                                notice::sendNoticeSMS($phone, 'Поставщик активировал полноценный режим работы: '.$GLOBALS['host'].$url);
//                            }
//
//                        }
//                    }
    ...
    /*$ulType = rrsIblock::getPropListId('farmer_profile', 'UL_TYPE', $arResult['SHOW_PROPS']['UL_TYPE']);
