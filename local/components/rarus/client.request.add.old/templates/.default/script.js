$(document).ready(function() {
    ...
    $('.submit-btn').on('click', function(e) {
        ...
        //check percent parameter
        checkObj = $('.additional_row.percent.active input[type="text"][name="percent"]');
        if (checkObj.length == 1) {
            temp_val = checkObj.val();
            if (temp_val == '') {
                err_val = 'Пожалуйста заполните это обязательное поле';
            }
            else if (!checkCorrectInt(temp_val) || parseInt(temp_val) < 1 || parseInt(temp_val) > 100) {
                err_val = 'Пожалуйста укажите целое положительное число от 1 до 100';
            }
            ...
            if (err_val != '') {
                ...
                if (checkObj.parents('.row').find('.additional_row.percent').find('.row_err').length == 1) {
                    checkObj.parents('.row').addClass('error').find('.additional_row.percent').find('.row_err').text(err_val);
                }
                else {
                    checkObj.parents('.row').find('.additional_row.percent').append('<div class="row_err"></div>');
                    checkObj.parents('.row').addClass('error').find('.additional_row.percent').find('.row_err').text(err_val);
                }
            }
            err_val = '';
        ...
        checkObj = $('.additional_row.delay.active input[type="text"][name="delay"]');
        if (checkObj.length == 1) {
            temp_val = checkObj.val();
            if (temp_val == '') {
                err_val = 'Пожалуйста заполните это обязательное поле';
            }
            else if (!checkCorrectInt(temp_val) || parseInt(temp_val) < 1) {
                err_val = 'Пожалуйста укажите целое положительное число';
            }
            ...
            if (err_val != '') {
                ...
                if (checkObj.parents('.row').find('.additional_row.delay').find('.row_err').length == 1) {
                    checkObj.parents('.row').addClass('error').find('.additional_row.delay').find('.row_err').text(err_val);
                }
                else {
                    checkObj.parents('.row').find('.additional_row.delay').append('<div class="row_err"></div>');
                    checkObj.parents('.row').addClass('error').find('.additional_row.delay').find('.row_err').text(err_val);
                }
            }
            err_val = '';
    ...
    $('.request_add select[name="payment"]').on('change', function(){
        if ($(this).val() == 81) {
